...
def sea_level_prediction(filepath_sea_level: str, filepath_co2: str, co2_input: float,
    ...
    co2_emissions = [co2_data[year] for year in co2_data]
    sea_levels = [sea_level_data[year] for year in sea_level_data]

    x_list = [sum(co2_emissions[0:i]) for i in range(1, len(co2_emissions) + 1)]
    y_list = sea_levels
...
def land_loss_prediction(filepath_land_loss: str, sea_level_rise: float,
    ...
    land_loss_points = regression_points(x_list, y_list, x_future, degree=3)
...
def pop_displacement_prediction(filepath_pop_displacement: str,
                                sea_level_rise: float, country_code: str) -> float:
    """
    ...
    pop_displacement_points = regression_points(x_list, y_list, x_future, degree=3)

    # pop_displacement_points is a list with one point
    # this check is to ensure that due to errors in regression,
    # the population displacement is never negative
...
def land_loss_national_stats(filepath_land_loss: str, sea_level_rise: float) -> List[float]:
    """
    Return a list of predicted national land-loss percentages
    in the same order of countries as in land_loss.csv.
    """
    land_loss_data = dataset_processing.process_land_loss(filepath_land_loss)

    national_land_loss = []
    for country_code in land_loss_data:
        national_land_loss.append(land_loss_prediction(filepath_land_loss,
                                                       sea_level_rise, country_code))

    return national_land_loss


def pop_displacement_national_stats(filepath_pop_displacement: str,
                                    sea_level_rise: float) -> List[float]:
    """
...
def show_graph(x_existing: List[float], y_existing: List[float],
    ...
    fig.update_layout(title=graph_titles[0],
                      xaxis_title=graph_titles[1],
                      yaxis_title=graph_titles[2]
                      )
...
if __name__ == '__main__':
    ...
    python_ta.check_all(config={
        'extra-imports': ['numpy', 'sklearn.linear_model', 'sklearn.preprocessing',
                          'plotly.express', 'plotly.graph_objects', 'typing',
                          'dataset_processing', 'python_ta.contracts'],
