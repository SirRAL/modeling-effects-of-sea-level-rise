...
def display_animated_graph(sea_level_points: List[float]) -> None:
    ...
    total_years = [datetime.date(2013, 1, 1).year + (1 + i)
                   for i in range(0, len(sea_level_points))]
    location = ['World'] * len(sea_level_points)
    data_dict = {"Location": location, "Year": total_years,
                 "Sea Level (mm)": sea_level_points
                 }
    ...
    fig = px.bar(df,
                 ...
                 range_y=[0, max(sea_level_points)]
                 )
    fig.show()
...
if __name__ == '__main__':
    ...
    python_ta.check_all(config={
        'extra-imports': ['plotly.express', 'pandas',
                          'datetime', 'typing', 'python_ta.contracts'],
